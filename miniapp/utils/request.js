...
/**
 * 常见英文错误消息的中文映射
 * 当后端返回的是英文错误消息时，优先展示对应的中文翻译
 */
const ERROR_MESSAGE_CN_MAP = {
  'book not found': '未找到该图书',
  'user not found': '未找到该用户',
  'loan not found': '未找到该借阅记录',
  'reservation not found': '未找到该预约记录',
  'unauthorized': '登录已过期，请重新登录',
  'forbidden': '没有权限执行此操作',
  'bad request': '请求参数有误',
  'internal server error': '服务器内部错误，请稍后重试',
  'no available copies': '当前没有可借副本',
  'already borrowed': '您已借阅此书',
  'already reserved': '您已预约此书',
  'max renewals reached': '已达到最大续借次数',
  'overdue books exist': '您有逾期图书未归还',
}
...
function parseErrorMessage(payload, fallback) {
  ...
  if (validationErrors) {
    ...
    if (firstField && validationErrors[firstField]) {
      return String(validationErrors[firstField])
    }
  }

  const rawMessage = payload.message || payload.error || fallback
  const cnMessage = ERROR_MESSAGE_CN_MAP[String(rawMessage || '').toLowerCase()]

  return cnMessage || rawMessage
}
