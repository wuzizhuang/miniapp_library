...
Page({
  ...
  async cancelReservation(event) {
    ...
    if (!reservationId || this.data.cancellingReservationId) {
      return
    }

    const targetReservation = (this.data.items || []).find(
      (item) => Number(item.reservationId) === reservationId,
    )
    const confirmed = await confirmAction({
      title: '确认取消预约',
      content: `确认取消《${(targetReservation && targetReservation.bookTitle) || '当前图书'}》的预约吗？`,
      confirmText: '确认取消',
    })
