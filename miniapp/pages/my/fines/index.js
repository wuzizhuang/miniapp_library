...
Page({
   ...
   */
  async payFine(event) {
    const fineId = Number(event.currentTarget.dataset.fineId || 0)

    if (!fineId || this.data.payingFineId) {
      return
    }

    const targetFine = (this.data.items || []).find((item) => Number(item.fineId) === fineId)
    const confirmed = await confirmAction({
      title: '确认支付罚款',
      content: `确认支付《${(targetFine && targetFine.bookTitle) || '当前图书'}》的罚款 ¥${(targetFine && targetFine.amountText) || '0.00'} 吗？`,
      confirmText: '确认支付',
    })
